...
export default {
  ...
  methods: {
    submitFeedbackForm() {
      // if form check need to be done, when submit btn clicked, (instead of @input="..." on each input) ---> this.$v.$touch();
      if (this.$v.$invalid) {
        return
      } else {
        // submit btn (id="submitFeedback__btn") modification after form's submition
        ...
        setTimeout(() => {
          ...
          axios.post(`${process.env.API_URL}/api/feedback/`, feedbackFormData, {
            headers: {
